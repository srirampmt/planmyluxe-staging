    ...
    <>
      ...
      {variant === "headerRow" ? (
        <div className="flex items-center gap-2 flex-nowrap">
          {!!badges?.length && (
            <div className="flex items-center gap-2 overflow-x-auto scrollbar-hide flex-1 min-w-0">
              {badges.map((badge, index) => (
                <div
                  key={`${badge}-${index}`}
                  className={`text-[12px] leading-[140%] px-3 py-[8px] rounded-full flex items-center gap-1 font-bold shadow-sm shrink-0 whitespace-nowrap ${
                    index === 0
                      ? "bg-pml-primary text-white shadow-pml-primary/30"
          ...
          <button
            ...
            className={`${className} ml-auto shrink-0 hidden md:flex items-center gap-2 bg-[#E8E5E5] text-[#585959] px-[10px] md:px-[32px] py-[10px] md:py-[6px] font-bold rounded-full shadow-md transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:bg-[#4C4C4C] active:scale-95 text-[14px] leading-[140%]`}
          >
            <Share2 className="w-4 h-4" />
            <span className="hidden md:block">Share This Offer</span>
          </button>
        </div>
      ) : variant === "icon" ? (
        <button
          onClick={() => setOpen(true)}
          aria-label="Share this offer"
          className={`${className} flex h-9 w-9 items-center justify-center rounded-full bg-white border border-gray-200 text-[#595858] shadow-sm hover:bg-gray-50 transition-colors`}
        >
        ...
        <button
          onClick={() => setOpen(true)}
          aria-label="Share this offer"
          className={`${className} ml-auto flex items-center gap-2 bg-[#E8E5E5] text-[#585959] px-[10px] md:px-[32px] py-[10px] md:py-[6px] mb-5 font-bold rounded-full shadow-md transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:bg-[#4C4C4C] active:scale-95 text-[14px] leading-[140%]`}
        >
          <Share2 size={16} />
          <span className="hidden md:block">Share This Offer</span>
        </button>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          {/* Modal box */}
          <div className="relative bg-white w-[80%] md:w-[450px] rounded-2xl shadow-2xl p-4 z-10">
            {/* Header */}
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-extrabold text-xl text-pml-primary">
                Share via
              </h3>
              <button
                onClick={() => setOpen(false)}
                aria-label="Close share dialog"
                className="rounded-full p-1.5 hover:bg-pml-primary/10"
              >
