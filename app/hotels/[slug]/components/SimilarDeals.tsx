  ...
  const deals: TrendingCarouselDeal[] = hotels.map((d: any) => ({
    ...
    starting_price:
      d?.starting_price === null || d?.starting_price === undefined
        ? null
        : String(d.starting_price),
  }));
  return (
    <section className="w-screen relative left-[50%] right-[50%] ml-[-50vw] mr-[-50vw] font-['Montserrat']" >
      <div className="w-full max-w-[1440px] mx-auto px-[16px] sm:px-[24px] md:px-[32px] lg:px-[40px] py-[20px] md:py-[36px]">
        <div className="w-full max-w-[1280px] mx-auto">
          {/* Header */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-3 md:mb-5">
            <h2 className="text-[24px] md:text-[36px] font-extrabold text-[#242F40] leading-[30px] md:leading-[60px] tracking-tight max-w-[626px]">
              {title}
            </h2>

            <a
              href="/all-offers"
              className="text-pml-primary text-xs font-semibold underline hover:text-[#a81a6f] self-start md:self-end mt-2 md:mt-0"
            >
                ...
                return (
                  <CarouselItem key={idx} className="basis-auto">
                    <div className="flex-[0_0_auto] w-[270px] sm:w-[300px] md:w-[360px] h-[436px] font-['Montserrat']">
                      <div
                        className="cursor-pointer bg-white rounded-2xl overflow-hidden flex flex-col h-full shadow-md border border-gray-200 transition-all duration-200 hover:-translate-y-1 hover:shadow-xl hover:border-pml-primary/20 group"
